<div class="flex justify-center absolute right-12 top-5 z-20 rounded-lg">
    <div x-data="{
        ...
        <button x-ref="button" x-on:click="toggle()" :aria-expanded="open" :aria-controls="$id('dropdown-button')"
            ...
            @auth
                <span class="mr-2 hidden md:block">{{ __('Hi, ') }}{{ auth()->user()->name }}</span>
            @endauth
        ...
        <div x-ref="panel" x-show="open" x-transition.origin.top.left x-on:click.outside="close($refs.button)"
            ...
            <a @click="setTheme('theme-light')"
                ...
                class="cursor-pointer flex items-center gap-1 w-full rounded px-6 py-2 text-left text-base hover:bg-primary-600 hover:text-on-primary-50 disabled:text-gray-500">
